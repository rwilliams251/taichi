...
class VideoManager:
    ...
    def make_video(self, mp4=True, gif=True):
        ...
        command = (
            (get_ffmpeg_path() +
             f" -loglevel panic -framerate {self.framerate} -i ") +
            os.path.join(self.frame_directory, FRAME_FN_TEMPLATE) + " -s:v " +
...
def interpolate_frames(frame_dir, mul=4):
    ...
    for i, img in enumerate(images_interpolated):
        cv2.imwrite(f"interpolated/{i:05d}.png",
                    img * 255.0)  # pylint: disable=E1101
